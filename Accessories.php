...
// ទាញយកផលិតផលដែលជាប្រភេទ Accessories
$sql = "SELECT * FROM products WHERE category = 'Accessories' ORDER BY id DESC";
...
<body>
    ...
    <script>
        ...
        function toggleFavorite(productId, element) {
            ...
            .then(res => res.json())
            .then(data => {
                if (data.status === 'added') element.querySelector('i').className = 'fas fa-heart';
                else if (data.status === 'removed') element.querySelector('i').className = 'far fa-heart';
                else if (data.status === 'not_logged_in') window.location.href = 'login.php';
            });
        }

        // --- ✨ JavaScript សម្រាប់ដំណើរការ Quick View Modal ---
